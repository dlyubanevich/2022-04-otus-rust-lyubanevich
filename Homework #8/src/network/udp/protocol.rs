...
pub fn send_f32(socket: &UdpSocket, address: impl ToSocketAddrs, data: f32) -> SendResult {
    ...
    while sended_len < bytes.len() {
        ...
        sended_len += socket.send_to(buffer_slice, &address)?;
    }
    Ok(())
}

pub fn receive_f32(socket: &UdpSocket) -> Result<f32, Box<dyn std::error::Error>> {
    let mut buffer = [0; 4];
    ...
    while received_len < buffer.len() {
        ...
        received_len += socket.recv(buffer_slice)?;
    }
